...
const STATUS_LABELS = {
  NEW: 'Pedido recebido',
  PRESCRIPTION_PENDING: 'A aguardar receita',
  IN_VALIDATION: 'Em validação',
  AWAITING_PHARMACY: 'A confirmar disponibilidade',
  AWAITING_CLIENT: 'A aguardar a sua confirmação',
...
export function TrackingPage() {
    ...
    return (
      <div className="page-wrap-xs py-20 text-center">
        ...
        <p className="text-slate-500 text-sm mb-8 max-w-xs mx-auto">
          Verifique o link ou fale com a equipa para confirmar o estado do pedido.
        </p>
        <div className="space-y-3 max-w-xs mx-auto">
          <Link to="/" className="btn-secondary-lg w-full justify-center">Voltar ao início</Link>
          <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-green-500 px-5 py-3 text-white font-semibold hover:bg-green-600 transition-colors">
            <WhatsAppIcon />
            Falar no WhatsApp
          </a>
        </div>
  ...
  const statusHistory = (() => {
    try {
      return Array.isArray(order.status_history)
    ...
    <div className="min-h-[82svh] bg-[linear-gradient(180deg,#f8fffe_0%,#ffffff_30%,#ffffff_100%)]">
      <div className="page-wrap-xs py-8 pb-safe">
        ...
        <div className="mb-5">
          ...
          <p className="text-sm text-slate-500 mt-1">Veja o estado atual e os próximos passos do processo.</p>
        </div>
        ...
        {order.status === 'AWAITING_CLIENT' && (
          <div className="rounded-2xl border border-orange-200 bg-orange-50 p-5 mb-4">
            ...
            <p className="text-sm text-orange-700 leading-relaxed mb-4">
              O preço final já foi preparado pela equipa. Fale connosco no WhatsApp para confirmar e avançar com a entrega.
            </p>
            <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-orange-500 px-5 py-2.5 text-white text-sm font-semibold hover:bg-orange-600 transition-colors">
              <WhatsAppIcon />
          ...
          <div className="card p-5 mb-4 border border-slate-200">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between border-t border-slate-100 pt-2 mt-1 font-extrabold text-slate-950">
                ...
                <span className="text-teal-700">{fmt(order.total_price)}</span>
              </div>
            </div>
          </div>
        )}
        ...
        {dataUpdatedAt && (
          <p className="text-xs text-center text-slate-400 mb-4 font-medium">
            Última atualização: {fmtDate(new Date(dataUpdatedAt).toISOString())}
          </p>
        )}

        <div className="space-y-3">
          <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="w-full inline-flex items-center justify-center gap-2 rounded-2xl bg-green-500 px-6 py-4 text-white font-semibold hover:bg-green-600 transition-colors">
            <WhatsAppIcon />
          ...
          {(isDelivered || isCancelled) && (
            <Link to="/medicamentos" className="btn-secondary-lg w-full justify-center">
              Fazer novo pedido
            </Link>
          )}
        </div>
